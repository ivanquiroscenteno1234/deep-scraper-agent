...
# BOLT ⚡: Define search indicators as module-level constants to prevent recreation on each function call
_SEARCH_INDICATORS = (
    # AcclaimWeb patterns
...
_SEARCH_INDICATORS_LOWER = tuple(indicator.lower() for indicator in _SEARCH_INDICATORS)
...
async def node_analyze_mcp(state: AgentState) -> Dict[str, Any]:
    ...
    # BOLT ⚡: Hoist `page_content.lower()` and `raw_html.lower()` outside of checks/loops
    # to avoid redundant O(N) allocations for large strings (~100KB)
...
"""
    ...
    try:
        ...
        if has_search_inputs and not decision.is_search_page and not decision.is_results_grid:
            ...
            # BOLT ⚡: Use hoisted lowercased raw_html

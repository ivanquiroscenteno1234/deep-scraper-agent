...
async def node_capture_columns_mcp(state: AgentState) -> Dict[str, Any]:
    ...
    Bolt ⚡ Optimization:
    - Uses browser-side DOM filtering (get_filtered_html_with_indices)
    ...
    # Bolt ⚡: Get filtered HTML and visible indices directly from browser
    # This avoids fetching full HTML and processing regex in Python
    result_data = await browser.get_filtered_html_with_indices()
    filtered_html = result_data.get("html", "")
    visible_indices = result_data.get("visible_indices", [])

    # Still clean for LLM (truncation, etc) but heavy lifting is done in browser

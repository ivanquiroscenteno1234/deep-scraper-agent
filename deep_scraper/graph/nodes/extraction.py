...
async def node_capture_columns_mcp(state: AgentState) -> Dict[str, Any]:
    ...
    # Bolt ⚡ Optimization:
    # Use browser-side filtering to get pre-cleaned HTML and visible indices.
    # This reduces network payload and avoids heavy regex processing in Python.
    filtered_html, visible_indices = await browser.get_filtered_html_with_indices()

    if not filtered_html:
        # Fallback if something fails (shouldn't happen with proper MCP)
        snapshot = await browser.get_snapshot()
        filtered_html = snapshot.get("html", str(snapshot))

    # Further clean for LLM (truncation, etc.)

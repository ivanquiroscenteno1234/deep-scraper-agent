...
async def node_capture_columns_mcp(state: AgentState) -> Dict[str, Any]:
    ...
    # Bolt ⚡: Use browser-side filtering instead of Python regex
    # This fetches HTML with hidden elements already removed and calculates visible indices
    filtered_html, visible_indices = await browser.get_filtered_html_with_indices()

    # Fallback if filtered fetch failed
    if not filtered_html:
        log.warning("Browser-side filtering failed, falling back to raw snapshot")

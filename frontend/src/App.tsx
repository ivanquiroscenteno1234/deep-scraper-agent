...
function App() {
  ...
  const loadScripts = useCallback(async () => {
    ...
    try {
      ...
      // We purposefully don't depend on selectedScriptPath here to avoid loops
      // We only want to set it if it's currently empty
      if (scripts.length > 0) {
         setSelectedScriptPath(prev => prev || scripts[0].path);
      }
    } catch (err) {

...
## 2024-05-25 - Hoisting String Operations

**Learning:** Repeatedly calling `.lower()` on large strings (100KB+) inside a loop or generator expression (e.g., `any(x in content.lower() for x in list)`) causes massive temporary allocation and CPU overhead (O(N*M)).

**Action:** Hoisting the `.lower()` call outside the loop (`content_lower = content.lower()`) reduced execution time by 40% (1.65x speedup) in `deep_scraper/graph/nodes/navigation.py`. Always look for heavy transformations inside loops.
